...
class SystemFiles {
    ...
    this.remixd.event.register('notified', (data) => {
      if (data.scope === 'systemfiles') {
        if (data.name === 'created') {
          this.init(() => {
            this.event.trigger('fileAdded', [this.type + '/' + data.value, false])
          })
        } else if (data.name === 'removed') {
          this.init(() => {
            this.event.trigger('fileRemoved', [this.type + '/' + data.value])
          })
          ...
          this.remixd.call('systemfiles', 'get', {path: data.value}, (error, content) => {
            if (error) {
              ...
              this.event.trigger('fileExternallyChanged', [this.type + '/' + data.value, content])
            }
          })
        }
      }
  ...
  rename (oldPath, newPath) {
    ...
    this.remixd.call('systemfiles', 'rename', {oldPath: unprefixedoldPath, newPath: unprefixednewPath}, (error, result) => {
      if (error) console.log(error)
      this.init(() => {
        this.event.trigger('fileRenamed', [this.type + '/' + unprefixedoldPath, this.type + '/' + unprefixednewPath])
      })
    })
    return true
  }
